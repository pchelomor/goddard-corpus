...
### Issue ISS-008: L0105 Registry/Progress Entry Has No Canonical Physical Object
- **Affected Entry:** `L0105` (*Live In The End*, registry `full_date` 1968-07-19).
- **Description:** `corpus_registry.json` and `corpus_progress.json` report L0105/the lecture extraction queue as completed, but the current `main` repository tree contains no L0105 folder, `master_text_*`, source manifest, candidate set, worker report, or processing marker. The complete root Git tree is not truncated and moves directly from `[Lessons Q & A]` to `[Love Endureth]`; direct plausible `Live In The End` contents paths return 404 and repository code search finds no L0105 source files.
- **Editorial impact:** A `SOURCE_FIRST_CANONICAL` pass is impossible without a canonical physical source. No editorial files may be fabricated and L0105 must not be marked complete merely because registry/progress metadata says `COMPLETED`.
- **Resolution:** Mark L0105 **BLOCKED / MISSING_CANONICAL_OBJECT**. A future source-repair pass must restore or establish an unambiguous L0105 canonical folder/master tied to corpus ID L0105 and then run the full editorial cycle. This editorial pass does not rewrite the raw registry/progress metadata.
